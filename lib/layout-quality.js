...
function checkParentContainment({ entries, placementById, policy, groupPairs, checks }) {
  ...
  for (const entry of entries) {
    ...
    if (groupPairs.has(pairKey(entry.placementId, parentId))) {
      continue;
    }
...
function buildLayoutReview(input) {
  ...
  checkParentContainment({ entries, placementById, policy, groupPairs, checks });
