...
const { processGeneratedAsset } = require("./generated-asset-quality");

const DEFAULT_CODEX_BIN = "codex";
const DEFAULT_FOUNDATION_ASSET_TYPES = ["panel", "card_frame", "button"];
const MAX_PROMPT_ZONE_SUMMARIES = 8;
const HANDOFF_JOB_SCHEMA = "game-screen-foundry.imagegen-handoff.v3";
...
function normalizeInputImage(item, fallbackRole = "style_reference") {
  if (!item) {
    return null;
  }
  if (typeof item === "string") {
    return { path: item, role: fallbackRole };
  }
  if (!item.path) {
    return null;
  }
...
function buildInputImages(preset, options = {}) {
  const images = [];
  if (options.currentImagePath) {
    images.push({
      path: options.currentImagePath,
      role: "edit_target",
      purpose: "Modify this adopted asset while preserving all unrequested visual properties."
    });
  }
  for (const item of options.inputImages || []) {
    const normalized = normalizeInputImage(item);
    if (normalized) {
      images.push(normalized);
    }
  }
  for (const item of preset.referenceImages || []) {
    const normalized = normalizeInputImage(item, "style_reference");
    if (normalized) {
      images.push(normalized);
    }
  }
...
function buildPreserveRules(asset, dimensions, options = {}) {
  const exportRequirements = asset.exportRequirements || {};
  const rules = [
    `final pixel size ${dimensions}`,
    `asset role ${asset.assetType} / ${asset.role}`,
    "screen placement coordinates and layer order",
    "runtime text lanes and child-content slots",
    "all properties not named in Change only"
  ];
  if (exportRequirements.transparent) {
    rules.push("transparent background and clean isolated silhouette");
  }
  return unique([...rules, ...listValues(options.preserve)]);
}

function buildAcceptanceChecks(asset, dimensions) {
  const exportRequirements = asset.exportRequirements || {};
  const checks = [
    "Final output is a readable PNG raster asset, not a placeholder or wireframe.",
    `Final PNG pixel size is exactly ${dimensions}.`,
    "Runtime text slots and child placement slots remain empty unless the asset explicitly owns baked text.",
    "The result is visually inspected before adoption and preserves every declared invariant."
  ];
  if (exportRequirements.transparent) {
    checks.push("The final PNG contains transparent pixels, at least one transparent corner, and no visible chroma-key residue.");
  }
  return checks;
}
...
function buildAssetPrompt({
  ...
  return [
    `Use case: ${operation === "edit" ? "precise-object-edit" : "stylized-concept"}`,
    `Asset type: ${dimensions} game UI production asset`,
    `Primary request: ${operation === "edit" ? "Edit" : "Create"} ${asset.assetId} for ${input.screenKv.screenName}.`,
    `Operation: ${operation}`,
    inputImages.length
      ? `Input images: ${inputImages.map((item, index) => `Image ${index + 1}: ${item.role} at ${item.path}${item.purpose ? ` (${item.purpose})` : ""}`).join(" / ")}`
      : "Input images: none",
    change.length ? `Change only: ${change.join(" / ")}` : "Change only: create the requested asset from the declared specification.",
    preserve.length ? `Preserve: ${preserve.join(" / ")}` : "",
    exportRequirements.transparent
      ? "Scene/backdrop: isolated game UI asset on a removable flat chroma-key background when direct alpha output is unavailable."
      : "Scene/backdrop: use the declared screen and world context; do not introduce unrelated scenery.",
...
function buildAssetGenerationContract(input, assetId, options = {}) {
  const asset = input.materialSpecSheet.assets.find((candidate) => candidate.assetId === assetId);
  if (!asset) {
  ...
  const dimensions = getGenerationDimensions(asset, placement, getDesignRules(input.worldPreset));
  const inputImages = buildInputImages(input.worldPreset || {}, options);
  const operation = options.operation || (options.currentImagePath ? "edit" : "generate");
  const change = unique(listValues(options.change));
  const preserve = buildPreserveRules(asset, dimensions, options);
  const qualityPlan = buildAssetQualityPlan({ input, asset, placements });
  const compositionReview = options.compositionReview || buildCompositionReview(input);
  const layoutReview = options.layoutReview || buildLayoutReview(input);
  ...
  const acceptanceChecks = buildAcceptanceChecks(asset, dimensions);
  const transparent = Boolean(asset.exportRequirements && asset.exportRequirements.transparent);

  return {
    assetId,
    operation,
    source: options.source || "initial_generation",
    inputImages,
    change,
    preserve,
    ...
    transparencyPlan: {
      ...
      preferred: transparent ? "alpha_png" : "opaque_png",
      fallback: transparent ? "flat_green_chroma_key_then_remove" : "none"
    },
    postprocessPlan: {
      removeChromaKeyWhenDetected: transparent,
      normalizeToGenerationSize: true,
      rejectBeforeAdoptionOnFailure: true
    },
    acceptanceChecks,
    qualityPlan,
    compositionContexts,
...
function buildCodexPrompt(job) {
  return [
    ...
    "When the Codex imagegen Skill is available, use it as the preferred raster generation and editing workflow. Otherwise use an equivalent image-generation tool or return a blocker sidecar.",
    "Pass every generationContract.inputImages path to the image tool with its declared role; edit_target is the image to modify, while style_reference guides visual consistency.",
    "For edits, change only generationContract.change and preserve every generationContract.preserve invariant.",
    ...
    "Inspect every generated image before acceptance. If a declared invariant is visibly wrong, make one targeted retry instead of broad prompt expansion.",
    "For transparent assets, follow the imagegen Skill's built-in-first transparency workflow and remove any chroma key before final acceptance.",
    "Save each inspected final image exactly to its outputPath. The Foundry will then normalize size, verify alpha/chroma quality, and reject files that fail the acceptance gate.",
...
function buildImagegenJob(input, options = {}) {
  ...
  const assets = targetIds
    .map((assetId) => {
      ...
      const generationContract = buildAssetGenerationContract(input, assetId, {
        outputPath,
        compositionReview,
        layoutReview,
      ...
      return {
        ...
        generationSize: generationContract.generationSize,
        outputPath,
        blockerPath,
        generationContract,
        operation: generationContract.operation,
        inputImages: generationContract.inputImages,
        change: generationContract.change,
        preserve: generationContract.preserve,
        transparencyPlan: generationContract.transparencyPlan,
        postprocessPlan: generationContract.postprocessPlan,
        qualityPlan: generationContract.qualityPlan,
        qualityGate: {
          finalRasterRequired: true,
          placeholderPolicy: "blocked-sidecar-only",
          rejectBeforeAdoption: true,
          checks: generationContract.acceptanceChecks
        },
        compositionContexts: generationContract.compositionContexts,
        layoutContext: generationContract.layoutContext,
        prompt: generationContract.prompt,
...
function adoptImagegenOutputs(input, job) {
  ...
  for (const assetJob of job.assets) {
    if (fs.existsSync(assetJob.outputPath) && fs.statSync(assetJob.outputPath).isFile()) {
      const asset = assetById.get(assetJob.assetId);
      let qualityReport;
      try {
        qualityReport = processGeneratedAsset({
          filePath: assetJob.outputPath,
          asset,
          target: parseDimensions(assetJob.generationSize),
          apply: true,
          allowChromaKey: true,
          normalizeSize: true
        });
        ...
        qualityReport = {
          ...
          actions: [],
          checks: [{ status: "fail", code: "postprocess_failed", message: error.message }]
        };
      }
      qualityReports.push(qualityReport);
      if (!qualityReport.ok) {
        rejectedAssetIds.push(assetJob.assetId);
        continue;
      }
      ...
      nextInput.worldPreset.imagegenAssets[assetJob.assetId] = {
        ...
        generationContract: assetJob.generationContract,
        acceptance: {
          acceptedAt: new Date().toISOString(),
          checks: qualityReport.checks
        },
        postprocess: qualityReport.actions,
...
function classifyHandoffState({ job, runner, adoptedAssetIds, missingAssetIds, rejectedAssetIds, blockerReports }) {
  ...
  if (rejectedAssetIds.length && adoptedAssetIds.length === 0) {
    return "rejected";
  }
  if (blockerReports.length && adoptedAssetIds.length === 0) {
    return "blocked";
  }
  if (adoptedAssetIds.length > 0 && rejectedAssetIds.length > 0) {
...
function buildHandoffReport(job, runner, adoption) {
  const state = classifyHandoffState({
    ...
    rejectedAssetIds: adoption.rejectedAssetIds,
  ...
  const messages = {
    ...
    rejected: "Generated files exist, but the PNG acceptance gate rejected them.",
    blocked: "Image generation returned blocker sidecars instead of usable assets.",
    partial_rejected: "Some assets were adopted and some failed the PNG acceptance gate.",
  ...
  return {
    ...
    counts: {
      ...
      rejected: adoption.rejectedAssetIds.length,
    ...
    missingAssetIds: adoption.missingAssetIds,
    rejectedAssetIds: adoption.rejectedAssetIds,
    qualityReports: adoption.qualityReports,
    blockerReports: adoption.blockerReports
  };
}
...
function prepareImagegenWorkflow(input, { run = false, writeFiles = true, adoptOutputs = false } = {}) {
  ...
  const adoption = adoptOutputs || runner.ran
    ? adoptImagegenOutputs(input, job)
    : {
      nextInput: input,
      adoptedAssetIds: [],
      missingAssetIds: job.assets.map((asset) => asset.assetId),
  ...
  const qualityByAssetId = new Map(adoption.qualityReports.map((report) => [report.assetId, report]));
  const adoptedAssetIds = new Set(adoption.adoptedAssetIds);
  const rejectedAssetIds = new Set(adoption.rejectedAssetIds);
  const refreshedJob = {
    ...job,
    assets: job.assets.map((assetJob) => ({
      ...assetJob,
      status: adoptedAssetIds.has(assetJob.assetId)
        ? "accepted"
        : rejectedAssetIds.has(assetJob.assetId)
          ? "rejected"
          : blockersByAssetId.has(assetJob.assetId)
          ? "blocked"
          : fs.existsSync(assetJob.outputPath)
            ? "exists"
            : "missing",
      blocker: blockersByAssetId.get(assetJob.assetId) || null,
      acceptanceReport: qualityByAssetId.get(assetJob.assetId) || null
    }))
  };
