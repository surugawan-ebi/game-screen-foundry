...
function buildCraftPromptLines(designRules, asset, placement = null) {
  const style = getCraftStyle(designRules);
  if (!style) {
    return [];
  }
  const lines = [`Craft quality target: ${style.label}.`, ...style.promptLines];
  const maxDimension = placement ? Math.max(Number(placement.width || 0), Number(placement.height || 0)) : 0;
  if (maxDimension > 0 && maxDimension <= 72) {
    // Small parts (grabbers, tabs, chips) need a floor of craftsmanship, not
    // extra decoration.
    if (style.name === "painterly") {
      lines.push("Small part treatment: at this size, aim for minimum tactile quality — one soft core shadow, one crisp highlight, and a subtle material grain. Keep the form readable at final size; do not over-decorate.");
    } else if (style.name === "outlined_cel") {
      lines.push("Small part treatment: keep the same outline width as the larger sprites and simplify the interior to 2-3 decisive bands so the part stays crisp at final size.");
    }
  }
  return lines;
}
