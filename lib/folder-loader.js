...
function ensureWorkflowDefaults(bundle, { projectRoot = "", screenFolderPath }) {
  ...
  const workflow = bundle.worldPreset.imagegenWorkflow;
  const jobBaseDir = projectRoot || screenFolderPath;

  if (workflow.outputDir) {
    workflow.outputDir = resolveWorkflowPath(workflow.outputDir, screenFolderPath);
  } else {
    workflow.outputDir = path.join(screenFolderPath, "generated-assets");
  }

  if (workflow.jobDir) {
    workflow.jobDir = resolveWorkflowPath(workflow.jobDir, jobBaseDir);
  } else {
    workflow.jobDir = path.join(jobBaseDir, ".game-creative-generation", "imagegen-jobs");
  }
}
...
function registerGeneratedAssetsFromFolder(bundle, generatedAssetsDir) {
  ...
  const generatedFiles = walkFiles(generatedAssetsDir)
    .filter((filePath) => /\.(png|jpe?g|webp)$/iu.test(filePath))
  ...
  for (const filePath of generatedFiles) {
    ...
    autoRegistered.push(assetId);
  }
  return autoRegistered;
}

// Appends auto-registered generated PNGs to the screen folder's
// imagegen-assets.json so manually generated assets become part of the
// persistent manifest instead of living only in the loaded session.
function persistAutoRegisteredAssets(screenFolderPath, bundle, assetIds) {
  ...
  if (fs.existsSync(manifestPath)) {
    try {
      ...
      throw new Error(`imagegen-assets.json could not be parsed: ${error.message}`);
    }
  }
  const known = new Set(manifest.assets.map((item) => item && item.assetId).filter(Boolean));
  let written = 0;
  for (const assetId of assetIds) {
    if (known.has(assetId)) {
      continue;
    }
    const entry = bundle.worldPreset.imagegenAssets[assetId];
    if (!entry || !entry.path) {
      continue;
    }
    manifest.assets.push({
      assetId,
      path: path.relative(screenFolderPath, entry.path).split(path.sep).join("/"),
      backend: entry.backend || "folder_generated_asset",
      usesImagegen: entry.usesImagegen !== false,
      prompt: "Auto-registered from generated-assets scan."
    });
    written += 1;
  }
  if (written) {
...
function resolveScreenBundleFromFolder(resolved, projectContext = {}) {
  ...
  const autoRegisteredAssetIds = registerGeneratedAssetsFromFolder(bundle, path.join(resolved, "generated-assets")) || [];
