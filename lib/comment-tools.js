...
function getPriorityRank(priority) {
  return PRIORITY_ORDER[priority] !== undefined ? PRIORITY_ORDER[priority] : 3;
}

function selectCriticalAssetIds(progressRows) {
  const sortedRows = [...progressRows].sort((left, right) => {
    const priorityDelta = getPriorityRank(left.asset.visualPriority) - getPriorityRank(right.asset.visualPriority);
    if (priorityDelta !== 0) {
      return priorityDelta;
    }
    const leftCritical = CRITICAL_ROLES.has(left.asset.role) ? 0 : 1;
    const rightCritical = CRITICAL_ROLES.has(right.asset.role) ? 0 : 1;
    if (leftCritical !== rightCritical) {
      return leftCritical - rightCritical;
    }
    return String(left.asset.assetId).localeCompare(String(right.asset.assetId));
  });

  const criticalRows = sortedRows
    .filter((row) => row.asset.visualPriority === "high" || CRITICAL_ROLES.has(row.asset.role))
    .slice(0, 10);
  const fallbackRows = criticalRows.length
    ? criticalRows
    : sortedRows.slice(0, Math.min(6, sortedRows.length));
  return new Set(fallbackRows.map((row) => row.asset.assetId));
}

function pickCriticalAssetId(criticalAssetIds, assets) {
  const asset = assets.find((item) => criticalAssetIds.has(item.assetId)) || assets[0];
...
function buildHeuristicReview({ renderModel, revisionMap }) {
  ...
  const progressRows = assets.map((asset) => {
    ...
    const weight = PRIORITY_WEIGHT[asset.visualPriority] || 4;
    let score = 0;
  ...
  const criticalAssetIds = selectCriticalAssetIds(progressRows);
  ...
  if (revisedCount > 0 && imagegenCount === 0) {
    const assetId = pickCriticalAssetId(criticalAssetIds, assets);
    topFindings.unshift({
      assetId,
      severity: "high",
      title: "実画像生成に切り替える",
      message: "現在の一括生成はSVGモックなので、最終画面の描き込みや素材感には届いていません。高優先素材、背景、土台、CTAからimagegen生成に切り替えるべきです。",
      suggestedComment: "この画面のworld-presetに合わせてimagegen実画像で作り直し、シルエット、余白、素材感、奥行きを優先して"
    });
  } else if (imagegenCount > 0 && criticalImagegenCount < criticalAssetIds.size) {
    topFindings.unshift({
      assetId: missingCriticalAssetIds[0] || pickCriticalAssetId(criticalAssetIds, assets),
      severity: "high",
      title: "主要素材のimagegen化が不足",
      message: `実画像は入っていますが、主要UI素材がまだSVG生成寄りです。次は ${missingCriticalAssetIds.slice(0, 4).join(" / ")} をimagegen化するべきです。`,
      suggestedComment: "この画面のworld-presetと素材語彙に合わせて実画像で作り直し、フレーム、装飾、発光、余白の役割差を明確にして"
    });
  }
  ...
  if (revisedCount > 0 && imagegenCount === 0) {
    summary = "現在はSVGモック段階です。構造確認としては進んでいますが、最終画面の雰囲気評価としてはまだ低く、実画像生成への切り替えが必要です。";
  } else if (imagegenCount > 0 && criticalImagegenCount < criticalAssetIds.size) {
    summary = "一部の素材はimagegen実画像に切り替わっています。ただし主要UI素材の多くがまだSVG生成なので、画面品質の評価は中間段階です。";
  ...
  return {
    ...
    guardrails: [
      ...
      "画面品質は現在のasset一覧にある高優先素材と主要ロールの実画像生成を必須条件にする",
      "一度良くなった素材は早めに固定する",
      "背景よりも CTA と土台を先に詰める",
      "毎回フル引き直しではなく差分再生成を優先する",
      "販売素材レベルの基準は、特定アセットの模倣ではなく、シルエット、余白、縁取り、素材感、透過境界、実装しやすさで判断する"
    ]
  };
}
