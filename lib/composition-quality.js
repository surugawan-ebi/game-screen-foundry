...
function checkLayerFit({ group, placement, rootPlacement, rule }) {
  ...
  if (resolvedRule && resolvedRule.fit === "inside_root") {
    ...
    return insetMeetsMinimum(inset, resolvedRule.minInset)
      ? createCheck("pass", "layer_fit_inside_root", `${placement.placementId} stays inside the root with the required inset.`, {
          ...refs,
          inset
        })
      : createCheck("fail", "layer_fit_inside_root", `${placement.placementId} touches the root edge; increase margin or lower minInset.`, {
          ...refs,
          inset,
