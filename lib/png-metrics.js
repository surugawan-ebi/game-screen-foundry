...
function analyzePngData(parsed, options = {}) {
  ...
  // Craft metrics: silhouette outline treatment and cel-shading structure.
  // Commercial sprite sets read through (1) a consistent darker outline along
  // the silhouette boundary and (2) a small number of decisive luminance
  // bands instead of muddy soft gradients.
  let boundaryPixels = 0;
  let boundaryDarkPixels = 0;
  let boundaryLumSum = 0;
  let interiorLumSum = 0;
  let interiorPixels = 0;
  const lumBins = new Array(24).fill(0);
  let lumSamples = 0;
  const isOpaqueAt = (x, y) => {
    if (x < 0 || y < 0 || x >= width || y >= height) {
      return false;
    }
    return rgba[(y * width + x) * 4 + 3] > alphaThreshold;
  };
  for (let y = 0; y < height; y += step) {
    for (let x = 0; x < width; x += step) {
      const offset = (y * width + x) * 4;
      ...
      const lum = luminance(rgba, offset);
      lumBins[Math.min(23, Math.floor(lum * 24))] += 1;
      lumSamples += 1;
      const onBoundary = !isOpaqueAt(x - 1, y) || !isOpaqueAt(x + 1, y)
  ...
  const interiorMeanLum = interiorPixels ? interiorLumSum / interiorPixels : 0;
  if (boundaryPixels) {
    // Second pass over boundary samples to count "clearly darker" outline pixels.
    for (let y = 0; y < height; y += step) {
      for (let x = 0; x < width; x += step) {
        const offset = (y * width + x) * 4;
        if (rgba[offset + 3] <= alphaThreshold) {
        ...
        const onBoundary = !isOpaqueAt(x - 1, y) || !isOpaqueAt(x + 1, y)
          || !isOpaqueAt(x, y - 1) || !isOpaqueAt(x, y + 1);
        if (onBoundary && luminance(rgba, offset) < interiorMeanLum - 0.12) {
          boundaryDarkPixels += 1;
        }
      }
    }
  }
  const luminanceBandCount = lumBins.filter((count) => lumSamples && count / lumSamples >= 0.04).length;
  ...
  return {
    ...
    outlineCoverage: roundMetric(boundaryPixels ? boundaryDarkPixels / boundaryPixels : 0),
    outlineContrast: roundMetric(boundaryPixels ? Math.max(0, interiorMeanLum - boundaryLumSum / boundaryPixels) : 0),
    luminanceBandCount
  };
}
