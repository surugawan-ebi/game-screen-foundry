...
// How the asset's pixels are meant to render. "raster_art" (default) must be
// effectively opaque inside its silhouette; "translucent_effect" declares
// intentional semi-transparency (scrims, glows) and is exempt from the
// translucency and craft audits.
function getAssetRenderIntent(asset) {
  const exportRequirements = asset && asset.exportRequirements ? asset.exportRequirements : {};
  const declared = String(exportRequirements.renderIntent || "");
  return declared === "translucent_effect" ? declared : "raster_art";
}
...
function buildDesignRulesPromptLines(designRules, asset) {
  ...
  const lines = [];
  if (renderIntent === "translucent_effect") {
    lines.push("Render intent: intentional translucent effect (scrim, glow, vignette). Export with a real alpha gradient; opacity is part of the design.");
  } else {
    lines.push("Render intent: solid raster art. The body inside the silhouette must be fully opaque; only anti-aliased edges may be semi-transparent. Never leave ghost blobs, holes, or half-flattened effect layers.");
  }
  lines.push([
