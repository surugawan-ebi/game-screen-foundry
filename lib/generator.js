...
const { buildLayoutReview } = require("./layout-quality");
const { buildStructureLayerSvg, buildStructureOverlaySvg, buildStructurePreview } = require("./structure-preview");
...
function generateRenderModel(input) {
  ...
  const structure = buildStructurePreview(input);
  const layers = input.materialSpecSheet.placements
    .map((placement) => {
      ...
      const structureDepth = structure.depthByPlacement.get(placement.placementId) || 0;
      return {
        placementId: placement.placementId,
        assetId: placement.assetId,
        role: asset.role,
        src: renderAssetPreview(asset, placement, input.worldPreset, revision),
        structureSrc: buildStructureLayerSvg({
          placement,
          depth: structureDepth,
          maxDepth: structure.maxDepth,
          contentInset: (structure.contentInsetByPlacement.get(placement.placementId) || {}).content || null,
          frameInset: (structure.contentInsetByPlacement.get(placement.placementId) || {}).frame || null
        }),
        structureDepth,
        width: placement.width,
        height: placement.height,
    ...
    .concat((input.materialSpecSheet.contentOverlays || []).map((overlay) => {
      ...
      const structureOverlay = buildStructureOverlaySvg({
        overlay,
        width: box.width,
        height: box.height
      });
      return {
        ...
        structureSrc: structureOverlay.src,
        structureTextOverflow: structureOverlay.overflow,
        width: box.width,
        height: box.height,
        left: box.left,
        top: box.top,
  ...
  return {
    ...
    layoutQuality: layoutReview.summary,
    layoutChecks: layoutReview.checks
  };
}
